...
int integrate_rk4(Vec2 pos, Vec2 vel, Attractor* attractors, int n, double dt, int n_steps, double r_stop) {
    for (int step = 0; step < n_steps; ++step) {
        ...
        for (int i = 0; i < n; ++i) {
            Vec2 att_pos = {attractors[i].x, attractors[i].y};
            if (vec_norm(vec_sub(pos, att_pos)) < r_stop) return i;
        }
...
void generate_map(Attractor* attractors, int n, const char* integrator_name, int grid_size, double dt, int n_steps, double r_stop, const char* filename) {
    ...
    if (strcmp(integrator_name, "rk4") == 0)
        ...
        fprintf(stderr, "Intégrateur inconnu\n");
        exit(1);
    }
...
int main() {
    ...
    generate_map(attractors, 3, "rk4", grid_size, dt, n_steps, r_stop, "rk4.ppm");
    generate_map(attractors, 3, "symplectic", grid_size, dt, n_steps, r_stop, "symplectic.ppm");

    printf("Images générées : rk4.ppm et symplectic.ppm\n");
    return 0;
}
